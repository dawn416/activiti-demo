...
public class DocReceiveDemo {
    ...
    public static void main(String[] args) {
        ...
        List<Task> taskList1 = taskService.createTaskQuery().taskAssignee("officeLeader1").orderByTaskCreateTime()
                .desc().list();
        ...
        // 分管领导1审核
        List<Task> taskList2 = taskService.createTaskQuery().taskAssignee("leader1").orderByTaskCreateTime().desc()
                .list();
        System.out.println("taskList2 = " + taskList2);
        taskService.complete(taskList2.get(0).getId());

        // 分管领导2审核
        List<Task> taskList3 = taskService.createTaskQuery().taskAssignee("leader2").orderByTaskCreateTime().desc()
                .list();
        System.out.println("taskList3 = " + taskList3);
        taskService.complete(taskList3.get(0).getId());

        // 分管领导3审核
        List<Task> taskList4 = taskService.createTaskQuery().taskAssignee("leader3").orderByTaskCreateTime().desc()
                .list();
        System.out.println("taskList4 = " + taskList4);
        taskService.complete(taskList4.get(0).getId());

        // 办公室工作人员分发
        List<Task> taskList5 = taskService.createTaskQuery().taskAssignee("createUserId").orderByTaskCreateTime().desc()
                .list();
        ...
        // 承办科室1受理
        List<Task> taskList6 = taskService.createTaskQuery().taskAssignee("dept1").orderByTaskCreateTime().desc()
                .list();
        ...
        // 承办科室2受理
        List<Task> taskList7 = taskService.createTaskQuery().taskAssignee("dept2").orderByTaskCreateTime().desc()
                .list();
        ...
        List<HistoricActivityInstance> historicActivityInstances = processEngine.getHistoryService()
                .createHistoricActivityInstanceQuery().orderByHistoricActivityInstanceStartTime().asc().list();
